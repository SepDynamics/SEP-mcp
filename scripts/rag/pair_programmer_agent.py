    ...
    def _process_event(self, event):
        ...
        try:
            ...
            if chaos and chaos["collapse_risk"] == "HIGH":
                print(f"⚠️ WARNING: {filepath.name} is in PERSISTENT_HIGH state (score: {chaos['chaos_score']:.3f}).")
                print(f"Impending structural ejection detected!")
            elif chaos:
                print(f"✅ {filepath.name} chaos score: {chaos['chaos_score']:.3f} ({chaos['collapse_risk']})")
